...
def test_create_json_folder_param(tmp_path, precreate):
...
def test_get_dict_from_json_file_param(tmp_path, existing, expected):
...
def test_remove_json_file_param(tmp_path, precreate):
...
def test_clear_since_from_state_removes_pattern(input_str, expected):
...
def test_is_float_param(value, expected) -> None:
