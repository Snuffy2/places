"""Integration tests for display options rendering in Places sensor.

This test suite verifies that different CONF_DISPLAY_OPTIONS values
produce the expected sensor state (self._attr_native_value) after
OSM parsing has populated internal attributes. It uses a snapshot of
internal attributes representing the sensor state immediately after
`parser.parse_osm_dict()` completes inside `query_osm_and_finalize`.
"""
...
async def test_display_options_state_render(display_option: str, expected_state: str):
    """Test that a given CONF_DISPLAY_OPTIONS value renders the expected sensor state.

    Steps:
      * Instantiate a Places sensor with minimal config.
      * Inject internal attributes snapshot (post-OSM parsing).
      * Override display options per parameter.
      * Run process_display_options.
      * Assert self._attr_native_value equals expected_state.
    """
