...
This module contains pytest-based asynchronous tests for parsing OpenStreetMap (OSM) data
and verifying correct attribute handling in the Places custom component.
"""
...
async def test_set_attribution(osm_dict, expected_attr, expected_value, should_call):
    """Test that set_attribution sets ATTR_ATTRIBUTION when 'licence' is present, otherwise does not call set_attr."""
    sensor = MockSensor()
...
async def test_parse_namedetails_none(monkeypatch):
    """Should do nothing if namedetails is missing or None."""
    sensor = MagicMock()
    parser = OSMParser(sensor)
    osm_dict = {}
    await parser.parse_namedetails(osm_dict)
...
async def test_parse_namedetails_name_only(monkeypatch):
    """Should set place name if 'name' in namedetails and CONF_LANGUAGE is blank."""
    sensor = MagicMock()
...
async def test_parse_namedetails_name_and_language(monkeypatch):
    """Should set place name to language-specific value if language matches."""
    sensor = MagicMock()
...
async def test_parse_namedetails_multiple_languages(monkeypatch):
    """Should set place name to first matching language in CONF_LANGUAGE list."""
    sensor = MagicMock()
    sensor.is_attr_blank.return_value = False
...
async def test_parse_namedetails_language_not_found(monkeypatch):
    """Should not set language-specific name if not present."""
    sensor = MagicMock()
    sensor.is_attr_blank.return_value = False
