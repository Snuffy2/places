...
async def test_parse_type_variants(
    ...
    parser, sensor = osm_parser()
    sensor.get_attr.side_effect = lambda k: get_attr_value if k == ATTR_PLACE_TYPE else None
    ...
    if expect_clear:
        ...
        for attr, val in expected_set_calls:
            sensor.set_attr.assert_any_call(attr, val)
...
async def test_parse_category_variants(osm_parser, category, address, expect_calls):
    ...
    parser, sensor = osm_parser()
    sensor.get_attr.side_effect = lambda k: category if k == ATTR_PLACE_CATEGORY else None
    await parser.parse_category(osm_dict)
    if expect_calls:
        sensor.set_attr.assert_any_call(ATTR_PLACE_CATEGORY, category)
        # The place name is taken from the address mapping value
        # Use next(iter(address.values())) to get the sample value used in tests
        sensor.set_attr.assert_any_call(ATTR_PLACE_NAME, next(iter(address.values())))
    else:
        sensor.set_attr.assert_not_called()
...
async def test_parse_namedetails_variants(
    ...
    # Configure language lookup behavior on the sensor
    sensor.is_attr_blank.return_value = is_lang_blank
    if language_pref is not None:
        # Place language preference in attrs so MockMethod returns it when invoked
        sensor.attrs[CONF_LANGUAGE] = language_pref
...
async def test_set_place_name_no_dupe_param(
    ...
    parser, sensor = osm_parser()
    sensor.is_attr_blank.side_effect = is_blank
    sensor.get_attr_safe_str.side_effect = lambda k: current_name
    sensor.get_attr.side_effect = existing_get_attr
    await parser.set_place_name_no_dupe()
    if should_set:
        sensor.set_attr.assert_any_call(ATTR_PLACE_NAME_NO_DUPE, current_name)
    else:
        sensor.set_attr.assert_not_called()
