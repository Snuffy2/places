...
from .conftest import stub_in_zone, stubbed_parser, stubbed_sensor
...
async def test_do_update_flow_variants(
    ...
    if should_handle:
        ...
        updater.handle_state_update.assert_not_called()

    if should_rollback:
        updater.rollback_update.assert_awaited_once()
    else:
        updater.rollback_update.assert_not_called()
...
async def test_handle_state_update_sets_native_value_and_calls_helpers(
    ...
    with stubbed_updater(updater, [("get_extended_attr", {}), ("fire_event_data", {})]):
    ...
    updater.fire_event_data.assert_awaited_once()
...
async def test_query_osm_and_finalize_runs_parser_and_sets_last_changed(
    ...
    with stubbed_sensor(sensor, [("process_display_options", {})]):
        with stubbed_parser(
            ...
            with patch(
                "custom_components.places.update_sensor.OSMParser", return_value=mock_parser
            ):
...
async def test_rollback_update_triggers_helpers(
    ...
    if expect_dot:
        ...
        assert updater.change_dot_to_stationary.await_count == 0
    if expect_show:
        updater.change_show_time_to_date.assert_awaited()
    else:
        assert updater.change_show_time_to_date.await_count == 0
...
async def test_get_extended_attr_variants(
    ...
    with stubbed_updater(updater, [("get_dict_from_url", {})]):
        await updater.get_extended_attr()
    if expect_call:
...
async def test_get_extended_attr_node_triggers_wikidata_lookup(
    ...
    with stubbed_updater(updater, [("get_dict_from_url", {"side_effect": fake_get_dict})]):
        await updater.get_extended_attr()

    # Should have attempted at least the details lookup and then the wikidata lookup
    assert updater.get_dict_from_url.await_count >= 1
...
async def test_fire_event_data_includes_extended_and_attributes(
    ...
    sensor.get_attr.return_value = "ext"
...
async def test_get_dict_from_url_handles_network_error(
    ...
    # Simulate aiohttp.ClientError by patching ClientSession to raise when entering response
    class _BadCM2:
